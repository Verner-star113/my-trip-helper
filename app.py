...
st.write("Планируйте маршруты между городами и странами, рассчитывайте время выезда и получайте уведомления!")

st.divider()
st.sidebar.header("🗺️ Запланировать поездку")
trip_form = st.sidebar.form("add_trip_form_v5_final", clear_on_submit=True)

with trip_form:
    st.markdown("### 🛫 Пункт отправления")
    origin_city = st.text_input("Город отправления:", placeholder="Например: Томск")
    origin_address = st.text_input("Улица, дом (необязательно):", placeholder="Например: ул. Ленина, 10")

    st.markdown("### 🛬 Пункт прибытия")
    ...
    transport_mode = st.selectbox(
        "Способ передвижения:",
        ["На автомобиле (Driving)", "Общественный транспорт (Transit)", "Пешком / На велосипеде"]
    )
    notes = st.text_area("Дополнительные параметры:")
...
with tg_col1:
    tg_token = st.text_input("Вставьте токен вашего Telegram-бота:", type="password", key="tg_token_v5")
with tg_col2:
    tg_chat_id = st.text_input("Вставьте ваш личный Telegram ID чата:", type="password", key="tg_id_v5")
...
if not active_trips:
    ...
    for t_id, t_cit1, t_adr1, t_cit2, t_adr2, t_arr, t_mode, t_notes, t_status in active_trips:
        ...
        with st.container(border=True):
            ...
            st.caption(f"🗺️ Масштаб маршрута: {timing['scale_status']}")
            ...
            # БРОНЕБОЙНЫЙ АВТОМОБИЛЬНЫЙ НАВИГАТОР ПО ТРАССАМ
            route_points = [[lat1, lon1], [lat2, lon2]]

            try:
                # МАГИЯ ФИНАЛА: Добавляем geometries=geojson, чтобы сервер отдал чистые точки трассы по дорогам!
                clean_url = f"https://project-osrm.org{lon1},{lat1};{lon2},{lon2}?overview=full&geometries=geojson"
                response = requests.get(clean_url, timeout=5)

                if response.status_code == 200:
                    res_data = response.json()
                    if "routes" in res_data and len(res_data["routes"]) > 0:
                        # Аккуратно вытаскиваем координаты всех изгибов асфальта
                        geojson_geometry = res_data["routes"][0]["geometry"]["coordinates"]
                        # Переворачиваем [Долгота, Широта] обратно в [Широта, Долгота] для Folium
            ...
            with btn_col2:
                if st.button("📲 Отправить сводку на телефон", key=f"tg_send_{t_id}", type="secondary",
                    ...
                    if not tg_token.strip() or not tg_chat_id.strip():
                        st.error("❌ Сначала заполните настройки Telegram-уведомлений!")
                    else:
                        alert_msg = (
                        ...
                        if send_telegram_alert(tg_token, tg_chat_id, alert_msg):
                            st.success("🚀 Сводка успешно отправлена!")
                        else:
                            st.error("❌ Не удалось отправить. Проверьте VPN на ПК или правильность токена/ID.")
